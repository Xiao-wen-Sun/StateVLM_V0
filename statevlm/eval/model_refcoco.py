...
def init_model(model_name_or_path):
    if "MiniCPM-V" in model_name_or_path:
        ...
        # processor = StateVLMImageProcessor.from_pretrained(model_name_or_path, trust_remote_code=True)
    # print(f"model: {model}")    
...
def eval_model_refexp(args):
    ...
    # loading model
    model, tokenizer = init_model(model_name_or_path=model_args.model_name_or_path)
    # loading Data
    ...
    with torch.inference_mode():
        ...
        for batch in tqdm.tqdm(dataloader, f'Inference'):
            ...
            #                     try:
            #                         # giou = generalized_box_iou(predict_location, target_location)
            #                         # print(f"giou: {giou}")
            #                         # if giou > 0.5:
            #                         #     correct += 1
            #                         # else:
            ...
            #     # # evaluate the accuracy from the location decoder
            #     # emb_predict_location = locations.cpu()
            #     # print(f"emb texts: {texts}")
            #     # print(f"target_location: {target_location}")
            #     # print(f"emb predict_location: {emb_predict_location}")
            #     # emb_giou = generalized_box_iou(emb_predict_location, target_location)
            #     # print(f"emb giou: {emb_giou}")
            #     # if emb_giou > 0.5:
            #     #     emb_correct += 1
            #     # else:
            #     #     emb_failed += 1
            #     # print(f"Emb Correct: {emb_correct}, Failed: {emb_failed}, Accuracy: {emb_correct / (emb_correct + emb_failed)}")
            ...
            #         "predict_location": predict_location.tolist(),
            #     }
...
if __name__ == "__main__":
    ...
    parser.add_argument("--image_path", default="/data/sun/datasets/refcoco/coco_imgs/",type=str, help="Path to the image directory.")
    parser.add_argument("--model_embedding", default=False, help="Embedding Paradigm or not.")
    #parser.add_argument("--model_name_or_path", default="/data/sun/StateVLM/MiniCPM-V-2_6", help="model path.")
    parser.add_argument("--model_name_or_path", default="/data/sun/StateVLM/statevlm_seq/statevlm_seq_2000", help="model path.")
