...
def format_instance(lines, formatted_data, system_prompt):
        ...
        if experssion.startswith("a"):
            formatted_entry = {
                ...
                "conversations": [
                    {
                        "role": "system",
                        "content": system_prompt + " Follow instructions.",
                    },
                    {
                        "role": "user",
                    ...
                    {
                        "role": "assistant",
                        "content": "Here is " + experssion + " in the image.",
                        "bbox": bbox_num  
                    }
                ]
            }
            ...
            formatted_entry = {
                ...
                "conversations": [
                    ...
                    {
                        "role": "user",
                        "content": "<image>\nWhat is the location of the " + experssion + " in the image?",
                    },
                    {
                        "role": "assistant",
...
def format_single_data(test_dataset_names, save_root, current_path, system_prompt):
    ...
    for test_name in test_dataset_names:
        ...
        with open(f"{current_path}/datasets/refcoco3_annotations/{test_name}.jsonl", "r") as f:
            lines = f.readlines()
        formatted_data = format_instance(lines, formatted_data, system_prompt)
        
        save_path = save_root + test_name +"_dia_eva.json"
...
if __name__ == "__main__":
    ...
    # Increasing x moves right; y moves down. Bounding box: [x1, y1, x2, y2] where (x1, y1) is top-left, (x2, y2) is bottom-right.
    # '''
    system_prompt = '''A chat between a human and an AI that understands visuals.'''
    
    REC_ref3_train = current_path + "/datasets/refcoco3_annotations/REC_ref3_train.jsonl"  
    REC_ref3_train_dia = current_path +"/datasets/evaluation/REC_ref3_train_dia.json"  

    val_dataset_names = ["REC_refcoco_unc_val", "REC_refcoco+_unc_val", "REC_refcocog_umd_val"]
    REC_ref3_val_dia = current_path + "/datasets/evaluation/REC_ref3_val_dia_eva.json"  
    ...
    save_root = current_path + "/datasets/evaluation/" 
    # save_root = current_path + "/datasets/evaluation_prompt/"
    # for training
    # format_refcoco_to_dia(REC_ref3_train, REC_ref3_train_dia)
    # format_val_to_one(val_dataset_names, REC_ref3_val_dia, current_path)
    
    # for test
    format_single_data(test_dataset_names, save_root, current_path, system_prompt)
    # format_single_data(val_dataset_names, save_root, current_path, system_prompt)
