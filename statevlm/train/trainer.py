...
class StateVLMTrainer(Trainer):
    def compute_loss(self, model, inputs, return_outputs=False):
        ...
        if labels is not None:
            ...
            # add the new loss by sxw
            bboxes_loss = outputs.loss
            logits = outputs.logits.view(-1, self.model.config.vocab_size).contiguous()
            if torch.equal(bboxes_loss, torch.tensor(0).to(bboxes_loss.device)):
                labels = labels.view(-1).long().contiguous()
                labels = labels.to(logits.device)
                loss = loss_fct(logits, labels)
                print(f"Sequence loss: {loss}")
            else:
                # text_labels = inputs['text_labels'].view(-1).long().contiguous()
                # text_labels = text_labels.to(logits.device)
                # loss = loss_fct(logits, text_labels)
                labels = labels.view(-1).long().contiguous()
                labels = labels.to(logits.device)
                seq_loss = loss_fct(logits, labels)
                print(f"Sequence loss: {seq_loss}") 
                bbox_loss = outputs.loss 
                print(f"bbox loss: {bbox_loss}") 
                loss = seq_loss*1/5 + bbox_loss*4/5
                # loss = bbox_loss
            ...
            if is_sagemaker_mp_enabled():
                ...
                if has_labels or loss_without_labels:
                    with self.compute_loss_context_manager():
                        # print("Computing loss and outputs")
    ...
    def _save(self, output_dir: Optional[str] = None, state_dict=None):
        ...
        if self.tokenizer is not None:
            ...
            # checkpoint-10/tokenization_minicpmv_fast.py
