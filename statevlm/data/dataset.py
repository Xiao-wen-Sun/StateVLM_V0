    ...
    def __getitem__(self, i) -> Dict[str, torch.Tensor]:
        try:
            if isinstance(self.raw_data[i]["image"], str):
                images = self.image_path + self.raw_data[i]["image"]
                images_dict = { "<image>" : Image.open(images).convert("RGB") }
            ...
            ret = dict(
                    ...
                    labels=ret["target"],
                    text_labels=ret["text_target"],
                    loc_targets = ret["loc_target"],
...
def data_collator(examples, padding_value=0, max_length=2048):
    def trim_and_pad(seq, batch_first, padding_value):
        padded_sequence = pad_sequence([s[:max_length] for s in seq], batch_first=batch_first, padding_value=padding_value)
    ...
    text_targets = trim_and_pad(
        [example["text_labels"] for example in examples],
        batch_first=True,
        padding_value=-100,
    )
    loc_targets = trim_and_pad(
        [example["loc_targets"] for example in examples],
        batch_first=True,
        padding_value=-100,
    )
...
def conversation_to_ids(conversation, tokenizer, llm_type=None, new_schema=False, max_length=2048):
    ...
    """
    if llm_type == "qwen2":
        input_ids, context, text_context, bbox, raw_msg = conversation_to_ids_qwen2(conversation, tokenizer)
    # print(f"context: {context}")
    # print(f"text_context: {text_context}")
    ids = torch.from_numpy(np.hstack(input_ids, dtype=np.int32))
    context = torch.from_numpy(np.hstack(context, dtype=np.int8))
    text_context = torch.from_numpy(np.hstack(text_context, dtype=np.int8))
    
    if len(bbox) > 0:
        bbox = torch.from_numpy(np.hstack(bbox, dtype=np.float16))
    else:
        bbox = torch.tensor([])
    ...
    if input_ids.shape[-1] > max_length:
        ...
        text_context = text_context[:max_length]
    ...
    if torch.all(context):
        logger.error("No context tokens available to compute loss.")
        raise Exception("No context tokens available to compute loss.")
    
    # if torch.all(text_context):
    #     logger.error("No text_context tokens available to compute loss.")
    #     raise Exception("No text_context tokens available to compute loss.")
    ...
    # build text target from text context            
    text_target = torch.full_like(ids, -100, dtype=torch.int32)
    for i in range(1, len(ids)):
        if text_context[i] == 0:
            text_target[i - 1] = ids[i]
        if text_context[i] == 1 and text_context[i - 1] == 0:
            if hasattr(tokenizer, "eot_id"):
                text_target[i - 1] = tokenizer.eot_id
            else:
                text_target[i - 1] = tokenizer.eos_id
    # build image bound
    if new_schema:
...
def conversation_to_ids_qwen2(conversation, tokenizer):
    ...
    for idx, msg in enumerate(conversation):
        ...
        if role == "user":
            ...
            prefix = "assistant"
            bbox.append(msg["bbox"])
            chat.append({"role": prefix, "content": message})       
        raw_msg += " " + prefix + ": " + message  
    assert set([i['role'] for i in chat]) & set(['assistant'])
    # print(f"role: {role}, message: {message}, bbox: {bbox}")
    
    ret = tokenizer.apply_chat_template(chat, tokenize=False, add_generation_prompt=False)
    # tokenizing the conversation
    input_ids = tokenizer.apply_chat_template(chat, tokenize=True, add_generation_prompt=False)
    input_ids = np.array(input_ids)

    im_start_idxs = np.where(input_ids == tokenizer.convert_tokens_to_ids('<|im_start|>'))[0]
    assistant_idxs = np.where(input_ids == tokenizer.convert_tokens_to_ids('assistant'))[0]
    im_end_idxs = np.where(input_ids == tokenizer.convert_tokens_to_ids('<|im_end|>'))[0]
    box_start_idxs = np.where(input_ids == tokenizer.convert_tokens_to_ids('<box>'))[0]
    
    # get the context from the input_ids
    context = np.ones_like(input_ids, dtype=np.int8)
    for assistant_idx in assistant_idxs:
        if assistant_idx-1 in set(im_start_idxs):
            st = assistant_idx + 1
            for im_end_idx in im_end_idxs:
                if im_end_idx > st:
                    context[st: im_end_idx + 1] = 0
                    break
    # get the text context from the input_ids            
    text_context = np.ones_like(input_ids, dtype=np.int8)
    for assistant_idx in assistant_idxs:
        if assistant_idx-1 in set(im_start_idxs):
            st = assistant_idx + 1
            for box_start_idx in box_start_idxs:
                if box_start_idx > st:
                    text_context[st: box_start_idx -1] = 0
                    break
                    
    input_ids = np.hstack(input_ids)
    context = np.hstack(context)
    text_context = np.hstack(text_context)
    return input_ids, context, text_context, bbox, raw_msg
...
def make_supervised_evaluation_data_module(
    tokenizer: transformers.PreTrainedTokenizer,
    data_args,
    transform,
    ...
    """Make dataset and collator for supervised fine-tuning."""
    dataset_cls = SupervisedEvalDataset
    ...
    if data_args.test_data_path:
        ...
        return dict(
            test_dataset=test_dataset,
            data_collator_eval= partial(data_collator_eval, max_length=max_length),
        )
    else:
        assert data_args.test_data_path, "eval_data_path must be provided"
        
class SupervisedEvalDataset(Dataset):
    """Dataset for supervised evaluation."""
    ...
    def __getitem__(self, i) -> Dict[str, torch.Tensor]:
        try:
            ...
            if isinstance(self.raw_data[i]["image"], str):
                ...
                ### for multi-images input, the template for every image is <image_xx>, such as <image_00>, <image_01>
                images_dict = {img_name : Image.open(img_path).convert("RGB") for img_name, img_path in self.raw_data[i]["image"].items()}
            image_id = self.raw_data[i]["image"]
            image_size = images_dict['<image>'].size
            # ret = {
                # input_ids, 
                # position_ids, 
                # labels, 
...
def data_collator_eval(examples, padding_value=0, max_length=2048):
    def trim_and_pad(seq, batch_first, padding_value):
        padded_sequence = pad_sequence([s[:max_length] for s in seq], batch_first=batch_first, padding_value=padding_value)
        return padded_sequence
    ...
    image_bound = [example["image_bound"] for example in examples]
    tgt_sizes = [example["tgt_sizes"] for example in examples]
    image_id = [example["image_id"] for example in examples]
    image_size = [example["image_size"] for example in examples]
    return {
        "input_ids": input_ids,
        "position_ids": position_ids,
        "text_labels": text_targets,
...
def preprocess_eval(
    ...
    if len(images_dict) == 1 and "<image>" in images_dict:       
        if "<image>" in conversations[1]["content"]:
            conversations[1]["content"] = conversations[1]["content"].replace(
                "<image>", image_placeholder
            )
        else:
        ...
        input_dict = conversation_to_ids_eval(conversations, tokenizer, llm_type, new_schema, max_length)
    else:
        pattern = r'<image_\d+>'
        new_conversations = []
        for conversation in conversations:
...
def conversation_to_ids_eval(conversation, tokenizer, llm_type=None, new_schema=False, max_length=2048):
    """
    for single image multi-turn conversation
    conversation: [{'role': 'system', 'content': 'This is a human robot interact model.'},
                   {'role': 'user', 'content': 'Describe this image'},
                ]"""
    if llm_type == "qwen2":
        input_ids, assistant_context, bbox, raw_msg = conversation_to_ids_qwen2_eval(conversation, tokenizer)
   
    ids = torch.from_numpy(np.hstack(input_ids, dtype=np.int32))
    
    encoded_context = [ord(char) for char in assistant_context]
    # Convert the encoded values into a torch tensor
    assistant_context = torch.tensor(encoded_context, dtype=torch.int32)
...
def conversation_to_ids_qwen2_eval(conversation, tokenizer):
    raw_msg = ""
    chat = []
    assistant_context = []
    bbox = []
    ...
    input_ids = np.array(input_ids)
    input_ids = np.hstack(input_ids)
    return input_ids, assistant_context, bbox, raw_msg
